...
def synthesize_demo_payload(
    ...
    Design goals (these are the teaching points):
      * MID_PRICE prints on most listed strikes near the money
        ...
        for strike in strikes:
            for cp in ("C", "P"):
                ...
                for ts, spot in df_stock["TRDPRC_1"].items():
                    ...
                    # exchange mid is a marked price; last trade is noisier / stickier
                    trade = mid * rng.normal(1.0, 0.06) + rng.normal(0, 0.02)
                    trade = max(trade, 0.01)

                    # Liquidity mask: mid almost always exists near the money;
