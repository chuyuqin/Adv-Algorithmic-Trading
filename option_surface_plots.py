...
DARK = dict(
    ...
    paper_bgcolor="#14110f",
    plot_bgcolor="#1e1a17",
    font=dict(color="#f2ede6", family="monospace"),
)
...
def candlestick_figure(df_stock: pd.DataFrame, ticker: str) -> go.Figure:
    fig = go.Figure(
        data=[
            go.Candlestick(
                ...
                increasing_line_color="#f2b84b",
                increasing_fillcolor="#f2b84b",
                decreasing_line_color="#4fb0a8",
                decreasing_fillcolor="#4fb0a8",
                name=ticker,
            )
        ]
    )
    fig.update_layout(
        ...
        paper_bgcolor="#14110f",
        plot_bgcolor="#1e1a17",
        font=dict(color="#f2ede6", family="Inter, system-ui, sans-serif", size=12),
        title=dict(
        ...
        xaxis=dict(gridcolor="#3a332d", rangeslider=dict(visible=False)),
        yaxis=dict(gridcolor="#3a332d", title="Price ($)"),
        ...
        annotations=[
            dict(
                ...
                y=1.02,
                showarrow=False,
                font=dict(size=11, color="#a89a8c"),
            )
        ],
    )
    return fig
...
def price_surface_figure(
            ...
            if grid is not None:
                fig.add_trace(
                    go.Surface(
                        ...
                        colorscale=[[0, "#0d3d38"], [0.5, "#1a7a6e"], [1, "#f2b84b"]],
                        opacity=0.28,
                        showscale=False,
                        hoverinfo="skip",
                        contours=dict(
        ...
        fig.add_trace(
            go.Scatter3d(
                ...
                marker=dict(
                    ...
                    opacity=1.0,
                    symbol="diamond",
                    line=dict(width=0.5, color="#7fd4cb"),
                ),
                hovertemplate=(
    ...
    fig.update_layout(
        ...
        title=dict(
            ...
            font=dict(size=16, color="#f2ede6", family="Inter, system-ui, sans-serif"),
            x=0.02,
            xanchor="left",
            y=0.98,
            yanchor="top",
        ...
        annotations=[
            dict(
                text="Amber = exchange MID_PRICE &nbsp;·&nbsp; Teal = last trade (TRDPRC_1) &nbsp;·&nbsp; Sheet is interpolated, not a market",
                xref="paper",
                yref="paper",
                x=0.0,
                y=1.0,
                ...
                yanchor="bottom",
                showarrow=False,
                font=dict(size=11, color="#a89a8c", family="Inter, system-ui, sans-serif"),
            )
        ],
    )
    return fig
...
def mid_vs_trade_figure(wide: pd.DataFrame, asof=None, ticker: str = "UUUU") -> go.Figure:
    ...
    if len(both):
        color = both["cp"].map({"C": "#f2b84b", "P": "#6f9bd1"})
        fig.add_trace(
            go.Scatter(
                x=both["TRDPRC_1"],
                y=both["MID_PRICE"],
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=[lo - pad, hi + pad],
                mode="lines",
                line=dict(color="#a89a8c", dash="dash", width=1),
                name="y = x",
                showlegend=True,
            ),
            row=1,
    ...
    fig.add_trace(
        go.Bar(
            ...
            marker_color=["#f2b84b", "#c9a86b", "#4fb0a8"],
            showlegend=False,
    ...
    fig.update_layout(
        ...
        font=dict(color="#f2ede6", family="Inter, system-ui, sans-serif", size=12),
        title=dict(
            text=f"{ticker}  ·  last trade is not the mid",
            font=dict(size=16, family="Inter, system-ui, sans-serif"),
            x=0.02,
            xanchor="left",
            y=0.98,
        ...
        annotations=[
            dict(
                text="Off-diagonal = mark ≠ print &nbsp;·&nbsp; Amber calls, slate-blue puts &nbsp;·&nbsp; Bars: listed series that day",
                xref="paper",
                yref="paper",
                x=0.0,
                y=1.02,
    ...
    fig.update_annotations(font=dict(size=13, family="Inter, system-ui, sans-serif", color="#f2ede6"))
    return fig
...
def coverage_heatmap(wide: pd.DataFrame, asof, cp: str = "C", field: str = "TRDPRC_1") -> go.Figure:
    ...
    accent = "#f2b84b" if field == "MID_PRICE" else "#4fb0a8"
    fig = go.Figure(
        data=go.Heatmap(
            z=pivot.values,
            x=[f"{c:.2f}" for c in pivot.columns],
            y=list(pivot.index),
            colorscale=[[0, "#1e1a17"], [1, accent]],
            zmin=0,
            zmax=1,
            showscale=False,
            xgap=2,
