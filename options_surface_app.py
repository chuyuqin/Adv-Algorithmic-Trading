...
2. MID_PRICE (exchange mark) is not TRDPRC_1 (last trade).
...
class State(rx.State):
    ...
    def _rebuild_option_figs(self):
        ...
        if wide is None or wide.empty or not self.asof:
            empty = go.Figure()
            empty.update_layout(template="plotly_dark", paper_bgcolor="#14110f")
...
def _metric(label: str, value) -> rx.Component:
    return rx.card(
        rx.text(label, size="2", color="#a89a8c"),
        rx.text(value, size="6", color="#f2b84b", weight="bold"),
        bg="#1e1a17",
    ...
    return rx.container(
        rx.vstack(
            rx.hstack(
                ...
                rx.spacer(),
                rx.badge(State.status_msg, color_scheme="amber", variant="solid"),
                width="100%",
                align="center",
                padding_y="1rem",
            ),
            rx.text(State.data_note, color="#a89a8c", size="2"),
            rx.hstack(
            ...
            rx.hstack(
                rx.text("As-of date", color="#a89a8c", size="2"),
                rx.select(
                    State.asof_options,
                    value=State.asof,
                ...
                rx.text("Right", color="#a89a8c", size="2"),
                rx.select(
                    ["C", "P"],
                    value=State.cp,
            ...
            rx.text(
                "Amber dots = exchange MID_PRICE. Teal diamonds = last trade. "
